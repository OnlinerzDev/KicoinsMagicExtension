...
public class Kasa1Procedure {
   public static void execute(LevelAccessor world, Entity entity) {
      if (entity instanceof Player player) {
         ...
         ItemStack goldIngotStack = new ItemStack(Items.HONEY_BOTTLE);
         if (player.getInventory().contains(goldIngotStack)) {
            ...
            if (itemHandlerRef.get() != null) {
               ...
               for (int idx = 0; idx < itemHandler.getSlots(); idx++) {
                  ...
                  if (itemStackInSlot.getItem() == Items.HONEY_BOTTLE) {
                  ...
                  if (amountOfItemNeeded >= 16.0D && !itemBought) {
                     itemBought = true;

                     // Remove 10 GOLD_INGOT from the player's inventory
                     removeItemFromPlayer(player, goldIngotStack, 16);
