...
public class Kasaaa3Procedure {
    ...
    public static void execute(LevelAccessor world, Entity entity) {
        if (entity instanceof Player player) {
            ...
            ItemStack goldIngotStack = new ItemStack(Items.SHULKER_SHELL);
            if (player.getInventory().contains(goldIngotStack)) {
                ...
                if (itemHandlerRef.get() != null) {
                    ...
                    for (int idx = 0; idx < itemHandler.getSlots(); idx++) {
                        ...
                        if (itemStackInSlot.getItem() == Items.SHULKER_SHELL) {
                        ...
                        if (amountOfItemNeeded >= 5.0D && !itemBought) {
                            itemBought = true;

                            // Remove 10 GOLD_INGOT from the player's inventory
                            removeItemFromPlayer(player, goldIngotStack, 5);

                            // Give the player 1 KIDCOIN_1 item
                            ItemStack newStack = new ItemStack(KidcoinyModItems.KIDCOIN_ZIELONY_SCRAP3.get(), 1);
